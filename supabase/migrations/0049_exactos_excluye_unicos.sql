-- 0049_exactos_excluye_unicos.sql
--
-- Separa "marcador exacto" de "marcador único" como categorías DISJUNTAS en la
-- tabla de posiciones. Hasta ahora un acierto único contaba a la vez en
-- `marcadores_exactos` y en `unicas_acertadas` (un único siempre es exacto). Para
-- efectos de la polla los manejamos como dos cosas distintas: si el acierto fue
-- único, cuenta SOLO en Únicos, no también en Exactos.
--
-- Resultado: las tres categorías de acierto quedan disjuntas (suman sin solape):
--   * aciertos          → con puntos pero NO marcador exacto         (ya en 0048)
--   * marcadores_exactos → exacto y NO único                          (este cambio)
--   * unicas_acertadas   → exacto y único
--
-- El cambio aplica de forma consistente a:
--   * la columna de salida `marcadores_exactos`, y
--   * el criterio de desempate configurable 'exactos' (los 3 CASE del ORDER BY).
-- Idéntica a la 0048 salvo esas 4 ocurrencias del filtro de exactos, donde se
-- agrega `AND NOT pred.prediccion_unica`. `unicas_iniciales`/`exactos_iniciales`
-- (arranque manual) se mantienen: el admin ya los carga como cifras separadas.
...
create or replace view public."vwTablaPosiciones" as
 SELECT p.grupo_id,
    ...
    count(pred.id) FILTER (WHERE pred.goles_local = pa.goles_local AND pred.goles_visitante = pa.goles_visitante AND pa.estado = 'finalizado'::text AND NOT pred.prediccion_unica) + p.exactos_iniciales AS marcadores_exactos,
    row_number() OVER (PARTITION BY p.grupo_id ORDER BY (COALESCE(sum(pred.puntos_obtenidos), 0::bigint) + p.puntos_iniciales) DESC, (
        CASE rg.criterios_desempate[1]
            WHEN 'exactos'::text THEN count(pred.id) FILTER (WHERE pred.goles_local = pa.goles_local AND pred.goles_visitante = pa.goles_visitante AND pa.estado = 'finalizado'::text AND NOT pred.prediccion_unica) + p.exactos_iniciales
