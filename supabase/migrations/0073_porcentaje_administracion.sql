-- ============================================================================
-- Porcentaje de administración del pozo.
--
-- Añade `tblReglasGrupo.porcentaje_administracion`: la parte del pozo
-- (valor_apuesta × participantes) que se reserva para gastos de administración
-- y NO entra al reparto de premios. Los premios (1.º/2.º/3.º, que siguen sumando
-- 100%) se calculan sobre el pozo NETO = bruto − administración. El cálculo en
-- pesos vive en la UI (PanelFinanciero); aquí solo se persiste el porcentaje.
--
-- Cambio additivo y seguro: `default 0` → las pollas existentes no cambian de
-- comportamiento (sin administración). La LECTURA ya lo recoge sin tocar nada
-- (grupo_detalle serializa la fila con `to_jsonb(r)`) y la EDICIÓN también
-- (actualizarReglas hace un `update` con el objeto completo del schema). Solo
-- hay que recrear `crear_grupo` para que lea el campo nuevo del JSON de reglas.
-- ============================================================================

alter table public."tblReglasGrupo"
  add column if not exists porcentaje_administracion int not null default 0
  constraint "tblReglasGrupo_porcentaje_administracion_check"
    check (porcentaje_administracion >= 0 and porcentaje_administracion <= 100);

-- Mismo cuerpo que 0059, con `porcentaje_administracion` añadido al insert de
-- reglas (default 0 si el cliente no lo manda).
...
begin
  ...
  insert into public."tblReglasGrupo" (
    ...
    minutos_cierre_prediccion, porcentaje_administracion, criterios_desempate
  ) values (
    v_grupo_id,
    ...
    coalesce((p_reglas->>'porcentaje_administracion')::int, 0),
    coalesce(
