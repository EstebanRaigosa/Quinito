...
import { claveDiaBogota, formatearFechaHoraBogota } from "@/lib/utils/fechas";
...
export default async function AdminResultadosPage() {
  ...
  // Día actual en zona Bogotá (CLAUDE.md §3.5) para la bandera "Hoy" del filtro.
  const hoyClave = claveDiaBogota(new Date());
  ...
  // Cada partido viaja con su fase y bandera `esHoy` para que el panel filtre
  // en cliente sin re-render del server.
  const secciones: SeccionTorneo[] = [...porTorneo.entries()].map(
    ([torneoId, lista]) => ({
      ...
      partidos: lista.map((p) => ({
        id: p.id,
        fase: p.fase,
        esHoy: claveDiaBogota(p.fecha_hora) === hoyClave,
        nodo: !p.equipo_local || !p.equipo_visitante ? (
          <div
            key={p.id}
          ...
          <FilaResultado
            ...
            partido={{
              ...
              prorrogaLocal: p.prorroga_local,
              prorrogaVisitante: p.prorroga_visitante,
