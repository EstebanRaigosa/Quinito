...
- **`app/globals.css` tiene un bloque `@supports (-webkit-touch-callout: none)`** (proxy de iOS) con: `touch-action: manipulation` + `button *,a * { pointer-events:none }` (§6, mata el delay de 300 ms y hace que el tap caiga en el botón), `overscroll-behavior-y: contain` (§11) y **anulación de `backdrop-filter`** en `.backdrop-blur*` (§8). **No re-habilitar blur en iOS.**
  - ⚠️ **Las barras sticky/fijas (top bar, headers de sección, footers del wizard) usan fondo SÓLIDO (`bg-app`/`bg-surface`), NO `bg-app/95`.** Los colores del DS se definen `var(--token)` con hex plano (sin canales `<alpha-value>`), así que Tailwind v3 **descarta** las clases con modificador de opacidad (`/95`, `/90`, `/10`…) y NO emite `background-color` → la barra queda transparente y el contenido se ve al hacer scroll (bug iOS jun 2026). Como el blur ya está anulado, la translucidez no aportaba nada. **No volver a poner `/95` en estas barras.**
...
- **`html { overflow-x: hidden }`** (guard anti-desborde horizontal global). Si un elemento se pasa del ancho del viewport, en WebKit el `BottomNav` (`position:fixed; inset-x-0`) se ancla al ancho del **documento** y desaparece al hacer scroll. Clipando en la **raíz** se elimina el scroll horizontal sin crear un contenedor de scroll que rompa los `sticky` internos (seguro iOS 15+). **Es una red de seguridad, no excusa**: igual hay que corregir el desborde en origen (causa típica: `grid` sin `grid-cols-1` → columna implícita `auto`; o `truncate` en flex anidado sin `min-w-0` en cada nivel).
...
- **Guardado de predicción = `useMutation` con `mutationKey` global** (`FormularioPrediccion` + `lib/queries/mutaciones-predicciones.ts`): el `mutationFn` y los reintentos viven en `registrarDefaultsMutaciones` (providers) para que la mutación sea **reanudable desde la cola persistida** aunque el formulario se desmonte. Las variables van **autocontenidas** (incluyen `participanteId`+`partidoId`). `lib/queries` siguen siendo insert/update explícito (no `upsert`, por los grants de RLS). **No volver a poner el `mutationFn` inline en el componente** (rompe el resume).
- **Cola de predicciones persistida** (`lib/queries/mutaciones-predicciones.ts`): `dehydrate`/`hydrate` a `localStorage` (clave `pollota:mutaciones-pendientes:v1`) persistiendo **SOLO mutaciones pausadas** (`shouldDehydrateQuery:()=>false` — JAMÁS queries → cero PII de terceros en disco, §3.4). Se reanuda en arranque, al volver online (`onlineManager`) y en `pageshow`/`visibilitychange`. Sin esto, una predicción guardada sin red se perdía al navegar/suspender (bug funcional grave).
- **`onlineManager` propio** (`instalarOnlineManager`): eventos `online`/`offline` reales, no el flag estático. El **timeout del fetch** cubre el WiFi "mentiroso" de estadio.
- **Timeout en TODAS las requests Supabase** (`lib/supabase/fetch-timeout.ts`, inyectado en `client.ts`/`server.ts`/`middleware.ts`): sin esto, con red intermitente una request cuelga para siempre (spinner eterno) y nunca dispara retry/pausa. **Sin `AbortSignal.any`** (no existe en iOS < 17.4): la señal externa se encadena a mano. Browser 15 s, server 10 s, middleware 8 s.
- **Banner de conexión global** (`components/shared/BannerConexion.tsx`, montado en `app/(app)/layout.tsx`): `fixed`+safe-area (NO `sticky`), muestra estado de red + predicciones pendientes de enviar.
- **`lib/supabase/client.ts`**: `auth.flowType:'pkce'` explícito + `realtime.worker:true` + `global.fetch` con timeout.
- **`getUsuarioActual` usa `getClaims()`** (`lib/auth/usuario-actual.ts`): verifica el JWT localmente (sin round-trip a GoTrue) cuando hay claves asimétricas; cae a `getUser()` si no. El middleware ya hace la validación fuerte. Devuelve `{id,email}` (no el `User` completo).
- **`/unirse/[codigo]` y `/offline` son rutas públicas** en `lib/supabase/middleware.ts` (`esRutaPublica`/`RUTAS_PUBLICAS`) y el middleware propaga `?next=` al redirigir a login. **El middleware NO expulsa a login ante error de RED transitorio si hay cookie de sesión** (`errorRed && tieneCookieSesion` → deja pasar). No quitar.
- **Página `/offline` + fallback del SW** (`app/offline/page.tsx`, `app/sw.ts` con `fallbacks`, precacheada vía `additionalPrecacheEntries` en `next.config.ts`): las navegaciones usan `NetworkOnly` (nunca cachean → privacidad) pero al fallar la red sirven `/offline` (estática y neutra, sin PII) en vez del dinosaurio de Chrome. Subir `revision` al editar `/offline`.
...
- **Cierre de modales con "atrás" (Android back / swipe-back iOS) = `lib/utils/modal-historial.ts` + `useModalBackClose`.** Al abrir una modal se empuja una entrada de historial (sin cambiar la URL); "atrás" la consume y cierra la modal en vez de navegar. Cuatro sutilezas, cada una arregla un bug real (no revertir):
  1. **El hook se llama DENTRO del `Content` de Radix** (componente `CierreConAtras` en `components/ui/sheet.tsx` y `dialog.tsx`), **no** en el wrapper `SheetContent`/`DialogContent`. El wrapper está siempre montado en modales controladas (`<Sheet open=…>`), así que ahí el hook empujaría una entrada fantasma al montar la página, no al abrir. El `Content` de Radix sí monta/desmonta con `open` (vía `Presence`).
  2. **El `history.back()` de limpieza es DIFERIDO (`setTimeout(0)`)** para sobrevivir a React StrictMode (dev): StrictMode monta→desmonta→remonta el efecto en el mismo tick; sin diferir, el `back()` del desmontaje falso se entrelaza con el re-`pushState` y descoloca la pila → "atrás" navega en vez de cerrar. El re-montaje cancela el back diferido y reutiliza la entrada.
  3. **La decisión de limpiar compara por URL (`urlActual()`), NO por `history.state.__modalAtras`.** Next 16 reescribe `history.state` en un `useInsertionEffect` en cada cambio de estado del router (p. ej. `router.refresh()` de la tabla en vivo), borrando cualquier marcador propio.
  4. **`TabsConRefresh` preserva `window.history.state` en su `replaceState`** (no pasa `null`): nulificarlo borra los marcadores internos del App Router de Next y, al volver atrás desde una modal abierta en esa pestaña, Next trata la entrada como navegación real.
